...
- 新增窗口自动吸附：悬停高亮、单击选中，拖动仍可自由框选
- 优化截图启动路径：取消无条件等待，截屏、窗口识别与界面加载并行完成
- 修复文字输入后点击空白处会丢失的问题
- 画笔、图形、箭头、文字和马赛克分别记住自己的参数
- 颜色按钮实时显示当前颜色，新增自定义颜色
- 线条、笔刷、字号改为连续滑杆调节
...
- Windows 10 / 11 64 位：`SnapCut-1.1.0-windows-x64.exe`
- Apple Silicon Mac（M 系列，macOS 12 或更高版本）：`SnapCut-1.1.0-macos-arm64.dmg`
